...
ordersRouter.get("/buyer/purchases", validate(listOrdersSchema, "query"), async (req, res, next) => {
  try {
...
ordersRouter.get("/seller/sales", validate(listOrdersSchema, "query"), async (req, res, next) => {
  try {
    const { page, limit, status } = req.query as any;
